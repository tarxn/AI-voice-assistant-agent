...
output_stream = audio.open(format=FORMAT, channels=CHANNELS, rate=LAPTOP_SAMPLING_RATE, input=True, frames_per_buffer=CHUNK) #for laptop mic audio
# input_stream = audio.open(format=FORMAT, channels=CHANNELS, rate=LAPTOP_SAMPLING_RATE, output=True) #for laptop's speaker audio 
...
#                         decoded_audio = web_audio_decoded_chunk(twilio_audio, input_sample_rate=8000, output_sample_rate=44100)
#                         playback_audio(decoded_audio, sample_rate=44100)
#                     except Exception as e:
...
#                 message = await websocket.receive_text()
#                 data = json.loads(message)
#                 if data['event'] == 'media' and stream_sid:
#                     twilio_audio = data['media']['payload']
#                     decoded_audio = web_audio_decoded_chunk(twilio_audio, input_sample_rate=8000, output_sample_rate=44100)
#                     output_stream.write(decoded_audio)

#         await asyncio.gather(send_audio_to_twilio(), recieve_audio_from_twilio())
